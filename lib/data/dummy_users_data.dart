import 'package:carshare/models/address.dart';
import 'package:carshare/models/user.dart';

List<User> dummyUsers = [
  User(
    "11060338920",
    "6063687",
    "47992766776",
    "Eu sou Igor Ponchielli, e quero ajudar atornar essa comunidade forte e a crescer.\n\nAdoro ver negócios que são construídos em torno da ideia de comunidade e sei que não é fácil começar um novo segmento, especialmente quando você está apenas começando. Mas estou aqui para ajudar!",
    UserGender.male,
    Address("89107-000", "SC", "Pomerode", "Testo Rega", "Rua Horst Rauh", 403),
    id: "yiKoZG8TSUhv1fd7W572vAn46M02",
    email: "igor.ponchielli@gmail.com",
    firstName: "Igor Felipe",
    lastName: "Ponchielli",
  ),
  User(
    "55139786010",
    "102499883",
    "47985049059",
    "Eu sou o Rafael, vc sabia que eu falo alemão?",
    UserGender.male,
    Address(
        "89012-001", "SC", "Blumenau", "Victor Konder", "Rua São Paulo", 1147),
    id: "lRndMwYM46PRVRmSxlJo1dJxwJb2",
    email: "rafaelsteinlenzi@gmail.com",
    firstName: "Rafael Stein",
    lastName: "Lenzi",
  ),
  User(
    "88527046040",
    "509581912",
    "47982784009",
    "Meu nome é Eduarda. Sou Analista de Negocios na WEG e moro em Jaraguá do Sul.\n\nAdoro estar na natureza, especialmente quando está ensoralado e florido. Quando não estou na cidade, geralmente estou em viagens a trabalho.",
    UserGender.female,
    Address(
        "89259-480", "SC", "Jaraguá do Sul", "Vila Nova", "Rua Pomerode", 50),
    id: "P9O0G1Ul8BRixV6akx9uNLb5H6A2",
    email: "poliane.nardi@gmail.com",
    firstName: "Eduarda",
    lastName: "Nogueira",
  ),
  User(
    "55139786010",
    "509581912",
    "47985049059",
    "Eu sou o Ricardo, um cara muito legal",
    UserGender.male,
    Address(
        "89012-001", "SC", "Blumenau", "Victor Konder", "Rua São Paulo", 1147),
    id: "TgLAhXEwFndGYCoucrbdpafGc2k1",
    email: "kadovargas@gmail.com",
    firstName: "Ricardo",
    lastName: "Vargas",
  ),
];
